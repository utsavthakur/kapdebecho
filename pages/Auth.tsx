...
import { Mail, Lock, User, ArrowRight, ChevronLeft, AlertCircle, Loader2 } from 'lucide-react';
...
const Auth: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (activeTab === 'login') {
        await authService.loginAsCustomer(formData.email, formData.password);
      } else {
        ...
        <div className="w-full max-w-md">
          <div className="text-center mb-10">
            ...
            <p className="text-stone-500">
              {activeTab === 'login' ? 'Enter your details to access your account' : 'Create your account to start ordering'}
            </p>
          </div>

          <div className="flex bg-stone-100 p-1 rounded-lg mb-8">
            <button
              className={`flex-1 py-2.5 text-sm font-bold rounded-md transition-all ${activeTab === 'login' ? 'bg-white text-maroon-900 shadow-sm' : 'text-stone-500 hover:text-stone-700'}`}
              onClick={() => setActiveTab('login')}
            >
              Sign In
            </button>
            <button
              className={`flex-1 py-2.5 text-sm font-bold rounded-md transition-all ${activeTab === 'signup' ? 'bg-white text-maroon-900 shadow-sm' : 'text-stone-500 hover:text-stone-700'}`}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {activeTab === 'signup' && (
              <>
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-stone-600 uppercase">Phone Number</label>
                  <div className="relative group">
                    <input
                      type="tel"
                      required
                      className="w-full px-4 py-3 border border-stone-200 rounded-lg focus:ring-2 focus:ring-maroon-900 focus:border-maroon-900 transition-all outline-none bg-white"
            ...
            <Button type="submit" disabled={loading} className="w-full py-3.5 text-base flex justify-center items-center gap-2 shadow-lg shadow-maroon-900/10 hover:shadow-maroon-900/20 transform transition-all hover:-translate-y-0.5">
